...
const SITE_NAME = "travel-weaver-65"
  ...
  server: {
    handlers: {
      POST: async ({ request }) => {
        ...
        if (token === supabaseServiceKey) {
        ...
        } else if (templateName === 'welcome-signup') {
          const { data: userList } = await supabase.auth.admin.listUsers({ page: 1, perPage: 1000 })
          authorized = Boolean(userList?.users?.some((u: any) => u.email?.toLowerCase() === effectiveRecipient.toLowerCase()))
        } else if (templateName === 'booking-confirmation' && bookingId) {
          ...
          authorized = Boolean(
            booking?.id &&
              String(booking.customer_info?.email ?? '').toLowerCase() === effectiveRecipient.toLowerCase()
          )
        }
        if (!authorized) {
        ...
        // 5. Send immediately and keep the queue as a fallback for temporary failures.
        await supabase.from('email_send_log').insert({
          message_id: messageId,
          template_name: templateName,
          recipient_email: effectiveRecipient,
        ...
        // Resend disabled — use Lovable Emails only
        ...
        if (resendApiKey) {
          ...
          try {
            ...
            if (!resp.ok) {
              throw new Error(`Resend ${resp.status}: ${respBody.slice(0, 500)}`)
            }
            await supabase.from('email_send_log').insert({
              message_id: messageId,
              template_name: templateName,
            ...
            console.error('Resend send failed — falling back to Lovable Email', {
              error: errorMsg,
              templateName,
              recipient_redacted: redactEmail(effectiveRecipient),
            })
            await supabase.from('email_send_log').insert({
              ...
              recipient_email: effectiveRecipient,
              status: 'failed',
              error_message: `Resend: ${errorMsg.slice(0, 900)}`,
            })
            // fall through to Lovable Email fallback below
          }
        }

        // Fallback: Lovable Email
        try {
          ...
          console.error('Immediate email send failed; queueing for retry', {
            error: errorMsg,
            templateName,
            recipient_redacted: redactEmail(effectiveRecipient),
          })
